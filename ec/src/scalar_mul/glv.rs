...
use ark_ff::{biginteger::arithmetic as fa, BigInteger, PrimeField, Zero};
use ark_std::{
...
/// comparison; the GLV multiplications use the JSF variant by default.
pub fn binary_scalar_mul_shamir<G: SWCurveConfig>(
    b1: Projective<G>,
    k1: G::ScalarField,
    b2: Projective<G>,
    k2: G::ScalarField,
    ...
    let mut bits = iter_k1.zip(iter_k2);

    // Skip the leading all-zero columns and seed the accumulator from the first

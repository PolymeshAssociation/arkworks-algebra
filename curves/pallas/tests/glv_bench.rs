...
use ark_ec::{
    scalar_mul::glv::{
        binary_scalar_mul_jsf, binary_scalar_mul_jsf_affine, binary_scalar_mul_shamir,
        fast_scalar_decomposition, generic_scalar_decomposition, GLVConfig,
    },
...
fn jsf_vs_shamir() {
    let rng = &mut test_rng();
    let n = 1000;
    let b1: Vec<G> = (0..n).map(|_| G::rand(rng)).collect();
    let b2: Vec<G> = (0..n).map(|_| G::rand(rng)).collect();
    let k1: Vec<Fr> = (0..n).map(|_| Fr::rand(rng)).collect();
    let k2: Vec<Fr> = (0..n).map(|_| Fr::rand(rng)).collect();
